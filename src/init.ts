...
export const init = (token: string) => {
  ...
  client.on(Events.MessageUpdate, async (oldMessage, newMessage) => {
    // Only moderate if the content actually changed and it's not a partial
    if (newMessage.partial || !newMessage.content) return;
    if (!client.user) return;
    if (newMessage.author?.id === client.user.id) return;

    // Moderation: banned words on edit
    const moderationResult = await LanguageModeration.isContentSafe(
      newMessage.content
    );
    if (!moderationResult) {
      ...
      // Send warning embed and delete after 5 seconds
      const warningMsg = await newMessage.channel
        .send({
      ...
      try {
        ...
        console.error("Failed to log censored edited message:", err);
      }
    }
  });
